 ...
 * @param {number} [option.retries.duration] Max duration in msec, in which the retries
 * will stop.
 */
...
Broker.prototype._onTimer = function () {
    ...
    if (this._option.healthCheckInterval > 0) {
        ...
        if (this._healthCheckIn <= 0) {
            ...
            .then(function (res) {
                if (res[0] == 0) {
                    self.logger.debug('Broker (%s): healthcheck %d', self.id, res[0]);
                    return;
                }
                if (res[0] == 1) {
                    self.logger.debug('Broker (%s): healthcheck %d', self.id, res[0]);
                ...
                self.logger.info('Broker (%s): %s', self.id, res[1]);
            }).done();
        }
    }
